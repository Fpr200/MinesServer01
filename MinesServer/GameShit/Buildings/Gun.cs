...
namespace MinesServer.GameShit.Buildings
{
    public class Gun : Pack, IDamagable
    {
        ...
        {
            Console.WriteLine(p.crys[CrystalType.Cyan]);
            if (charge == maxcharge)
            {
                return;
            }
            using var db = new DataBase();
            db.Attach(this);
            if (p.crys[CrystalType.Cyan] < val) val = p.crys[CrystalType.Cyan];
            
            if (p.crys.RemoveCrys((int)CrystalType.Cyan, val))
            {
                
                charge += (int)val;
                if (charge - (int)val == 0) World.W.GetChunk(x, y).ResendPacks();
        ...
        {
            MButton[] fillbuttons = [new MButton("+100", "fill:100", (args) => Fill(p, (long)100)),
                new MButton("+1000", "fill:1000", (args) => Fill(p, (long)1000)),
                new MButton("max", "fill:max", (args) => Fill(p, (long)(maxcharge - charge)))
               ];
            return new Window()
            {
                Tabs = [new Tab()
