...
namespace MinesServer.GameShit.ClanSystem
{
    public class Clan
    {
        ...
        {
            GUI.MButton[] buttons = [new MButton("ПОКИНУТЬ", "leave", (args) => LeaveClan(p))];
        ...
        public void OpenPlayerPrew(Player p, Player target, bool changerank = false)
        {
            ...
            if (changerank)
            {
                ...
                buttons = [new MButton("ИСКЛЮЧИТЬ", "kick", (args) => KickPlayer(p, target))];
            }
            if (p.clanrank.priority > target.clanrank.priority)
            {
                if (changerank)
                {
                    buttons = buttons.Concat([new MButton("СОХРАНИТЬ", "saverank", (args) => OpenPlayerPrew(p, target, true))]).ToArray();
                }
                else
                {
                    buttons = buttons.Concat([new MButton("РЕДАКТИРОВАТЬ", "changerank", (args) => OpenPlayerPrew(p, target, true))]).ToArray();
                }
            }
        ...
        public static void CreateClan(Player p, int icon, string name, string abr)
        {
            ...
            {
                new Rank() { name = "Новичек",priority = 0,colorhex = "#00FF00",owner = c },
                new Rank() { name = "Глвав",priority = 20,colorhex = "#ff0000",owner = c },
            ...
            {
                p.connection?.SendU(new OKPacket("КЛАН НЕ СОЗДАН", $"Для создания клана требуется 1000 кредитов.\nУ вас {p.creds}"));
            }
        }
